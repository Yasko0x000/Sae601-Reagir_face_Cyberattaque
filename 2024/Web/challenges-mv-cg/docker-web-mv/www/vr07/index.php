...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['action'])) {
    if ($_GET['action'] == 'add' && !empty($_GET['message'])) {
        ...
        if (preg_match('/<script\b[^>]*>(.*?)<\/script>/is', $message) || preg_match('/shell_exec\((.*?)\)/is', $message) || preg_match('/<script\b[^>]*>/is', $message) || preg_match('/system\((.*?)\)/is', $message) || preg_match('/passthru\((.*?)\)/is', $message) || preg_match('/exec\((.*?)\)/is', $message) || strpos($message, 'dir') !== false || strpos($message, '*') !== false|| strpos($message, 'password') !== false) {
            // Ne pas ajouter le message et générer une alerte
            echo "<script>alert('Le contenu du message est interdit. Les balises script, le mot password sont entre autres interdits.');</script>";
